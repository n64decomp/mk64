...
static arg_config default_config = {
    .in_filename = NULL, .out_filename = NULL, .tmp_filename = NULL, .offset = 0x0, .compress = 0, .alpha_color = 0x01
};
...
static void print_usage(void) {
    ERROR("Usage: tkmk00 [-c / -d] [-o OFFSET] FILE [OUTPUT]\n"
          ...
          default_config.alpha_color, default_config.offset);
    exit(1);
}
...
static int extract_tkmk00(const char* in_filename, const char* out_filename, const char* tmp_filename, uint32_t offset,
                          uint32_t alpha_color) {
    FILE* out;
    uint8_t* in_buf = NULL;
...
int main(int argc, char* argv[]) {
    ...
    if (config.compress) {
        ...
        ret_val = extract_tkmk00(config.in_filename, config.out_filename, config.tmp_filename, config.offset,
                                 config.alpha_color);
    }
