...
int get_color_index(const rgba comp, const rgba* pal, int mask_value, int pal_size) {
    ...
    for (; pal_idx < pal_size; pal_idx++) {
        if (comp_rgba(comp, pal[pal_idx]) == 1) {
            return pal_idx;
}
    }
 ...
 * Sets the values of rawci (8 bit color index array) to the appropriate index in pal that each entry in img can be
 *found at If a value in img is not found in pal, return 0, indicating an error Returns 1 if all values in img are found
 *somewhere in pal
 **/
int imgpal2rawci(uint8_t* rawci, const rgba* img, const rgba* pal, const uint8_t* wheel_mask, int raw_size,
                 int ci_depth, int img_size, int pal_size) {
    int img_idx;
    int pal_idx;
    int mask_value;
...
static const format_entry format_table[] = {
    { "rgba16", { IMG_FORMAT_RGBA, 16 } }, { "rgba32", { IMG_FORMAT_RGBA, 32 } }, { "ia1", { IMG_FORMAT_IA, 1 } },
    { "ia4", { IMG_FORMAT_IA, 4 } },       { "ia8", { IMG_FORMAT_IA, 8 } },       { "ia16", { IMG_FORMAT_IA, 16 } },
    { "i4", { IMG_FORMAT_I, 4 } },         { "i8", { IMG_FORMAT_I, 8 } },         { "ci8", { IMG_FORMAT_CI, 8 } },
    { "ci4", { IMG_FORMAT_CI, 4 } },
};
...
static const scheme_entry encoding_table[] = {
    { "raw", ENCODING_RAW }, { "u8", ENCODING_U8 },   { "u16", ENCODING_U16 },
    { "u32", ENCODING_U32 }, { "u64", ENCODING_U64 },
};
...
static void print_usage(void) {
    ERROR("Usage: n64graphics -e/-i BIN_FILE -g IMG_FILE [-p PAL_FILE] [-o BIN_OFFSET] [-P PAL_OFFSET] [-f FORMAT] [-c "
          "CI_FORMAT] [-w WIDTH] [-h HEIGHT] [-V]\n"
          "\n"
          ...
          format2str(&default_config.format), encoding2str(default_config.encoding), default_config.width,
          default_config.height, format2str(&default_config.pal_format));
}
...
int main(int argc, char* argv[]) {
    ...
    switch (config.mode) {
        ...
        case MODE_EXPORT_CI: {
            ...
            conversion_success =
                imgpal2rawci(rawci, imgr, palr, wheel_mask, ci_length, config.format.depth, img_length, pal_length);
