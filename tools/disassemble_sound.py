...
def write_aiff(entry, filename):    
    ...
    try:
        ...
        print(f"WARNING: failed to matching decode {filename}, outputting raw aifc instead")
        with open(filename.replace(".aiff", ".aifc"), "wb") as aifc_output:
            write_aifc(entry, aifc_output)
        # Remove the half written aiff
        os.remove(filename)
    finally:
        temp.close()
        os.remove(temp.name)
...
def main():
    ...
    for bank_index, bank in enumerate(banks):
        ...
        with open(filename, "w") as out:
            ...
            for inst_index, inst in enumerate(bank.all_insts):
                if isinstance(inst, Inst):
                    ...
                    for drum in inst:
                        ...
                        addr_to_name[drum.addr] = drum.name
            ...
            # Drum lists
            for addr in bank.drum_list:
                if addr is None:
                    bank_json["drum_list"].append(None)
                else:
                    bank_json["drum_list"].append(addr_to_name[addr])
