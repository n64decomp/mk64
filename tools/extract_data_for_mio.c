...
static bool elf_get_section_range(uint8_t* file, const char* searched_name, uint32_t* address, uint32_t* offset,
                                  uint32_t* size, uint32_t* section_index) {
    ...
    for (int i = 0; i < u16be(ehdr->e_shnum); i++) {
        ...
        Elf32_Shdr* str_shdr =
            (Elf32_Shdr*) (file + u32be(ehdr->e_shoff) + u16be(ehdr->e_shstrndx) * u16be(ehdr->e_shentsize));
...
int main(int argc, char* argv[]) {
    ...
    for (uint32_t i = 0; i < symtab_size / sizeof(Elf32_Sym); i++) {
        ...
        printf("%08x\t%08x\t%02x\t%02x\t%02x\t%s\n", u32be(symbol->st_value), u32be(symbol->st_size), symbol->st_info,
               symbol->st_other, u16be(symbol->st_shndx), name);
#endif
    ...
    if (elf_get_section_range(file, ".mdebug", &mdebug_address, &mdebug_offset, &mdebug_size, &mdebug_index)) {
        ...
        for (uint32_t i = 0; i < u32be(symbolic_header->ifdMax); i++) {
            FileDescriptorTable* fdt =
                (FileDescriptorTable*) (file + u32be(symbolic_header->cbFdOffset) + i * sizeof(FileDescriptorTable));

            for (uint32_t j = 0; j < u32be(fdt->csym); j++) {
                LocalSymbolsEntry lse;
                memcpy(&lse,
                       file + u32be(symbolic_header->cbSymOffset) +
                           (u32be(fdt->isymBase) + j) * sizeof(LocalSymbolsEntry),
                       sizeof(LocalSymbolsEntry));
