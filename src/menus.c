...
s8 D_8018EDF1;
u8 gSoundMode;
s8 gNbPlayers; // main menu row id? but it is also the number of players define
s8 D_8018EDF4;
s8 D_8018EDF5;
s8 D_8018EDF6;
...
// set to gNbPlayers, then that sets gPlayerCountSelection1
...
void splash_menu_act(struct Controller *controller, u16 arg1) {
    ...
    if (func_800B4520() == 0) {
        ...
        gPlayerCountSelection1 = gNbPlayers = D_800F2B58[D_8018EDF1];
...
void func_800B28C8(void) {
    // For Grand Prix and Versus, this will be the CC mode selected. For Time Trials, it will
    // be whether 'Begin' or 'Data' is selected. Not used for Battle.
    s8 temp_v0 = D_800E86B0[gNbPlayers - 1][D_800E86AC[gNbPlayers - 1]];
    // Determine which game mode was selected based on the number of players and the row selected on the main menu
    switch (gGameModeFromNumPlayersAndRowSelection[gNbPlayers][D_800E86AC[gNbPlayers - 1]]) {
    case GRAND_PRIX:
...
void main_menu_act(struct Controller *controller, u16 arg1) {
    ...
    if (!func_800B4520()) {
        ...
        {
            newMode = gGameModeFromNumPlayersAndRowSelection[gNbPlayers][D_800E86AC[gNbPlayers - 1]];
            break;
        }
        case PLAYER_NUM_SELECTION:
        {
            if ((btnAndStick & R_JPAD) && gNbPlayers < 4) {
                gNbPlayers += 1;
                func_800B44AC();
                play_sound2(SOUND_MENU_CURSOR_MOVE);
            }
            if ((btnAndStick & L_JPAD) && gNbPlayers >= 2) {
                gNbPlayers -= 1;
                func_800B44AC();
                play_sound2(SOUND_MENU_CURSOR_MOVE);
            }
            // L800B2B38
            gPlayerCountSelection1 = gNbPlayers;
        ...
        {
            if (btnAndStick & D_JPAD) {
                if (D_800E86AC[gNbPlayers - 1] < D_800F2B58[gNbPlayers + 7]) {
                    D_800E86AC[gNbPlayers - 1] += 1;
                    func_800B44AC();
                    play_sound2(SOUND_MENU_CURSOR_MOVE);
                }
            }
        ...
        case TIME_TRIALS_DATA_SELECTION_FROM_BACK_OUT:
        {
            ...
            if ((arg1 == 0) && (++gMenuTimingCounter == 100 || gMenuTimingCounter % 300 == 0)) {
                // L800B2FAC
                if (gGameModeFromNumPlayersAndRowSelection[gNbPlayers][D_800E86AC[gNbPlayers - 1]] == 0 || gGameModeFromNumPlayersAndRowSelection[gNbPlayers][D_800E86AC[gNbPlayers - 1]] == 2) {
                    play_sound2(SOUND_MENU_SELECT_LEVEL);
                }
            }
            // L800B3000
            sp28 = D_800E86B0[gNbPlayers - 1][D_800E86AC[gNbPlayers - 1]];
            if ((btnAndStick & U_JPAD) && (sp28 > 0)) {
                D_800E86B0[gNbPlayers - 1][D_800E86AC[gNbPlayers - 1]] -= 1;
                func_800B44AC();
                play_sound2(SOUND_MENU_CURSOR_MOVE);
            }
            // L800B3068
            if (btnAndStick & D_JPAD) {
                sp24 = FALSE;
                if (func_800B555C()) {
                    if (sp28 < D_800F2B60[gNbPlayers + 4][D_800E86AC[gNbPlayers - 1] + 1]) {
                        sp24 = TRUE;
                    }
                } else {
                    // L800B30D4
                    if (sp28 < D_800F2B60[gNbPlayers][D_800E86AC[gNbPlayers - 1] + 1]) {
                        sp24 = TRUE;
                    }
                }
                // L800B3110
                if (sp24) {
                    D_800E86B0[gNbPlayers - 1][gNbPlayers] += 1;
                    func_800B44AC();
                    play_sound2(SOUND_MENU_CURSOR_MOVE);
                }
            }
            ...
            if (btnAndStick & B_BUTTON) {
                ...
                // L800B31DC
                func_800B44AC();
                if (gNbPlayers == 1 && D_800E86AC[gNbPlayers - 1] == 1 && sp28 == 1) {
                    func_8009E258();
                    play_sound2(SOUND_MENU_DATA);
                } else {
                    gMainMenuSelectionDepth = CONFIRM_OK_SELECTION;
...
void func_800B3F74(s32 menuSelection) {
    ...
    case 10:
    {
        ...
        D_8018EDF1 = D_800F2BDC[gNbPlayers + 3];
        func_800CA008(0, 0);
        func_800C8EAC(1);
        D_8018EDFC = 0;
        break;
