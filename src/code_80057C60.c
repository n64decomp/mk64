...
void func_80059820(s32 arg0) {
    ...
    D_8018CF1C = &gPlayerOne[arg0];
    D_8018CF14 = &camera1[arg0];
    temp_v0 = &D_8018CA70[arg0];
    temp_v0->posXInt = D_8018CF1C->pos[0];
    temp_v0->posYInt = D_8018CF1C->pos[1];
    temp_v0->posZInt = D_8018CF1C->pos[2];
}
...
extern s32 D_8018CE24[3][8];

void func_8005A14C(s32 playerId) {
    s32 unknown;
    s32 lapCount;
    Player *player;
    struct_80165C18_entry *entry;

    player = &gPlayerOne[playerId];
    unknown = D_8018CE24[playerId][0];
    lapCount = gLapCountByPlayerId[playerId];
    if (player->unk_000 & 0x8000) {
        entry = &D_80165C18[unknown];
        if (player->unk_0BC & 0x204C0) {
            entry->unk_0BE[2] += 0x1000;
        } else if (entry->unk_0BE[2]) {
            entry->unk_0BE[2] += 0x1000;
        }
        if (player->unk_0BC & 0x40000000) {
            f32_step_towards(&entry->unk_000, 0.3f, 0.02f);
        } else {
            f32_step_towards(&entry->unk_000, 0.6f, 0.02f);
        }
        if (player->unk_0BC & 0x04000000) {
            u16_step_up_towards(&entry->unk_0BE[0], 0x0C00, 0x0100);
        } else {
            u16_step_down_towards(&entry->unk_0BE[0], 0x0000, 0x0100);
        }
        if (player->unk_0BC & 0x03000000) {
            func_80087D24(unknown, 6.0f, 1.5f, 0);
        } else {
            f32_step_towards(&entry->unk_028[1], 0, 1.0f);
        }
        ...
        if (lapCount >= 3) {
            ...
            entry->unk_0BE[1] = 0;
            entry->unk_0BE[0] = 0;
            entry->unk_028[2] = 0.0f;
            entry->unk_028[1] = 0.0f;
            entry->unk_028[0] = 0.0f;
            entry->unk_000 = 0.6f;
            entry->unk_0A0 = 0xFF;
        }
    }
