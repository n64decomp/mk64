...
struct struct_D_802876D0 {
    ...
    /* 0x18 */ f32 unk18;
    /* 0x1C */ s8 unk1C;
  /* 0x00 */ f32  unk20;   // D_80287700[0] = D_80150130[0] (f32);
  /* 0x04 */ Vec3f unk24; // vec3f_set_dupe()
  /* 0x10 */ Vec3f unk30; // "
  /* 0x1C */ Vec3f unk3C; // "
  /* 0x28 */ Vec3s unk48;
  /* 0x2E */ Vec3s  unk4E;
...
void func_80283430(void)
{
    ...
    vec3f_clear(&D_802876E0[12]);
    vec3f_set_dupe(&D_802876E0, 0.0f, 0.0f, 500.0f);
    vec3f_clear(&D_80287700[4]);
    vec3f_set_dupe(&D_80287700[1], 0.0f, 0.0f, 500.0f);
    vec3f_set_dupe(&D_80287700[7], 0.0f, 1.0f, 0.0f);
    D_802876F8 = 0.0f;
    vec3s_clear(&D_80287728[0]);
    vec3s_clear(&D_80287728[1]);
    vec3s_clear(&D_80287728[2]);
    vec3s_clear(&D_80287728[3]);
    D_80287740.unk0 = 0;
...
s32 func_80283648(Camera *camera) {
    ...
    temp = (u8)func_8028336C(&D_802876E0, camera);
	D_802876FC = temp;
	...
	if (temp != 0)
	{
		...
		camera->up[0] = sins(var_f0) * coss(sp6C);
		camera->up[1] = coss(var_f0);
		camera->up[2] = -sins(var_f0) * sins(sp6C);
		vec3f_copy_dupe(camera->pos, &D_802876E0[12]);
		vec3f_copy_dupe(&camera->lookAt, (f32 *) (&D_802876E0));
	...
	vec3f_copy_dupe(&D_80287700[1], &camera->lookAt);
	vec3f_copy_dupe(&D_80287700[7], &camera->up);
	return D_802876D8;
}
#else
