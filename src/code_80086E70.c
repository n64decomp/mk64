...
s32 func_8008847C(s32 objectIndex) {
    ...
    if (is_obj_flag_status_active(objectIndex, 0x00000400) != 0) {
        ...
        gObjectList[objectIndex].unk_044 = func_802ABE30(gObjectList[objectIndex].pos[0], 0.0f, gObjectList[objectIndex].pos[2], D_8018C3B0.surfaceMapIndex);
    }
    return sp2C;
}
...
UNUSED s32 func_8008A870(Player *player) {
    return get_track_segment(player->unk_110.surfaceMapIndex);
}

s32 func_8008A890(Camera *camera) {
    return get_track_segment(camera->unk_54.surfaceMapIndex);
}
