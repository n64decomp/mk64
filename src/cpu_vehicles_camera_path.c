...
f32 time_crossed_finish_line(UNUSED s32 playerId, f32 previousPlayerZ, f32 playerZ) {
    ...
    return gCourseTimer -
           ((COURSE_TIMER_ITER_f * z_change_after_cross) / (z_change_after_cross + z_change_before_cross));
}
        ...
        if ((var_v1 != 0) && (playerZ <= gPathStartZ)) {
            if (gPathStartZ < previousPlayerZ) {
                ...
                if ((D_8016348C == 0) && !(player->type & PLAYER_CINEMATIC_MODE)) {
                    gTimePlayerLastTouchedFinishLine[playerId] =
                        time_crossed_finish_line(playerId, previousPlayerZ, playerZ);
                }
            }
        }
...
void update_player(s32 playerId) {
    ...
    if ((s32) GET_COURSE_AIMaximumSeparation >= 0) {
        ...
        if (!(player->lakituProps & HELD_BY_LAKITU) && !(player->lakituProps & LAKITU_SCENE)) {
            ...
            if ((gCurrentCourseId != COURSE_AWARD_CEREMONY) &&
                ((gCrossedFinishLine[playerId] == 1) || (playerId == 0))) {
                set_places();
            }
